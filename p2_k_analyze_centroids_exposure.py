#!/usr/bin/env python3
"""Overlay cyclone, riverflood, heat, cyclonetides, and landslide rasters with centroid GeoParquet by country.

For each centroids_{ISO3}_add_v2.parquet file, this script samples each asset
geometry against IRIS RP rasters and writes country-level parquet output(s)
named centroids_{ISO3}_exposure.parquet with exposure fields:
...
DEFAULT_CENTROIDS_DIR = Path(
    "/soge-home/projects/mistral/ji/bigdata_global_renewable_dataset_p1/2050_supply_100%_add_v2"
)
DEFAULT_OUTPUT_DIR = Path("output_per_country/parquet_centroids_exposure")
DEFAULT_GLOBAL_OUTPUT = Path("output_global/centroids_global_exposure.gpkg")
...
ISO3_RE = re.compile(r"^centroids_([A-Za-z0-9]{3})_add_v2\.parquet$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze centroid exposure using IRIS cyclone RP GeoTIFF maps."
    )
    parser.add_argument(
        "--hazard-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory containing centroids_{ISO3}_add_v2.parquet files",
    )
    parser.add_argument(
        "--riverflood-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Output directory for per-country centroid exposure parquet files",
    )
    parser.add_argument(
        "--global-output",
        type=Path,
        default=DEFAULT_GLOBAL_OUTPUT,
        help="Optional output path for combined global file (default: output_global/centroids_global_exposure.gpkg)",
    )
    parser.add_argument(
        "--rps",
        type=int,
...
def find_country_files(centroids_dir: Path, iso3_filter: set[str] | None) -> list[tuple[str, Path]]:
    files = sorted(centroids_dir.glob("centroids_*_add_v2.parquet"))
    if not files:
        raise FileNotFoundError(
            f"No centroid parquet files found in {centroids_dir} with pattern centroids_*_add_v2.parquet"
        )
    ...
    if not discovered:
        if iso3_filter:
            raise FileNotFoundError(
                f"No centroid files found for requested ISO3 list: {sorted(iso3_filter)}"
            )
        raise FileNotFoundError("No valid centroids_{ISO3}_add_v2.parquet files were found")
...
def sample_exposure_for_rp(gdf: gpd.GeoDataFrame, raster_path: Path) -> np.ndarray:
    with rasterio.open(raster_path) as src:
        if gdf.crs is None:
            raise ValueError("Input centroid data has no CRS metadata")
...
def process_country(
    ...
    out_file = output_dir / f"centroids_{iso3}_exposure.parquet"
...
def build_global_from_per_country(output_dir: Path, global_output: Path) -> None:
    files = sorted(output_dir.glob("centroids_*_exposure.parquet"))
    if not files:
        raise FileNotFoundError(
            f"No per-country centroid exposure files found in {output_dir}. "
...
def main() -> None:
    ...
    centroids_dir = resolve_path(script_dir, args.centroids_dir)
    ...
    country_files = find_country_files(centroids_dir, iso3_filter)
    ...
    print(f"Centroids directory: {centroids_dir}", flush=True)

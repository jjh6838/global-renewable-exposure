#!/bin/bash --login
#SBATCH --job-name=analyze_polylines_exposure
#SBATCH --output=/soge-home/users/lina4376/dphil_p2/p2_test/logs/slurm_analyze_polylines_exposure_%j.out
#SBATCH --error=/soge-home/users/lina4376/dphil_p2/p2_test/logs/slurm_analyze_polylines_exposure_%j.err
...
PY_SCRIPT=${SCRIPT_DIR}/p2_m_analyze_polylines_exposure.py
FINAL_OUTPUT_DIR=${SCRIPT_DIR}/output_per_country/parquet_polylines_exposure
GLOBAL_OUTPUT=${SCRIPT_DIR}/output_global/polylines_global_exposure.gpkg
POLYLINES_DIR=/soge-home/projects/mistral/ji/bigdata_global_renewable_dataset_p1/2050_supply_100%_add_v2
RPS=(10 100 500)
...
Usage:
	...
	bash p2_n_process_analyze_polylines_exposure.sh --write-global

	Explicit tiered submit:
	bash p2_n_process_analyze_polylines_exposure.sh --submit-tiered

  Worker mode (used by Slurm arrays):
	sbatch --array=0-(N-1) p2_n_process_analyze_polylines_exposure.sh --tier <1|2|3> --country-list-file <file>

  One-country test:
	bash p2_n_process_analyze_polylines_exposure.sh --iso3 ABW
...
if [[ ! -d "$POLYLINES_DIR" ]]; then
	echo "Polylines directory not found: $POLYLINES_DIR" >&2
	exit 1
fi

collect_all_iso3s() {
	find "$POLYLINES_DIR" -maxdepth 1 -type f -name 'polylines_*_add_v2.parquet' \
		-printf '%f\n' \
		| sed -E 's/^polylines_([A-Za-z0-9]{3})_add_v2\.parquet$/\1/' \
		| tr '[:lower:]' '[:upper:]' \
		| sort -u
}
...
files = sorted(input_dir.glob("polylines_*_exposure.parquet"))
if not files:
    raise SystemExit(f"No per-country polyline exposure files found in {input_dir}")
...
if (( SUBMIT_TIERED == 1 )); then
	...
	LIST_DIR="${SCRIPT_DIR}/output_global/slurm_country_lists_polylines/${RUN_TAG}"
	mkdir -p "$LIST_DIR"
	...
	if [[ "${#ALL_ISO3[@]}" -eq 0 ]]; then
		echo "No country polyline files found under $POLYLINES_DIR" >&2
		exit 1
	fi
	...
	SCRIPT_PATH="${SCRIPT_DIR}/p2_n_process_analyze_polylines_exposure.sh"
